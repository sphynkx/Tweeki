...
echo 'style="margin: 0; position: relative; left: -90px;"'; 
}endif;
?>
 class="<?php echo $contentclass; ?>">

<div class="row" 
<?php 
if($_SERVER['HTTP_SEC_CH_UA_PLATFORM']==='"Android"'): {
echo 'style="border: 0px solid #f00; position: relative; left:100px; overflow-x: hidden;"';
}endif;
?> >DIV_CLASS_ROW
...
<div style="border: 0px solid red; width=40px; height: 100px;">
  <div style="float:left;">
			<h1><span dir="auto"><?php $this->html( 'title_formatted' ) ?></span>
<?php
$shorturl = (isset( $this->get('sidebar')['TOOLBOX']['shorturl'] )) ? ' <sup><a href="' .$this->get('sidebar')['TOOLBOX']['shorturl']['href']. '" title="' .$this->get('sidebar')['TOOLBOX']['shorturl']['title']. '">&#128279;</a></sup>' : '';
echo $shorturl;
?>
</h1>
<?php
...
<?php
$content = $this->data[ 'bodycontent'];
/// mobile - NOT WORK
if($_SERVER['HTTP_SEC_CH_UA_PLATFORM']==='"Android"'):
$content = preg_replace("/VIDEO_ROWS=3/", "VIDEO_ROWS=1", $content);
endif;
/// /mobile
$content = preg_replace('|\n|','', $content);
preg_match_all('|<li class="toclevel-(.) tocsection.*?>.*?<span class="toctext">(.*?)</span>|u', $content, $content2);

$level = $content2[1];
$sect =  $content2[2];

if( count($level) > 4 ){
#echo count($level).'<a class="btnx btn-primaryx" data-toggle="collapse" href="#collapsedTOC" aria-expanded="false">';
#echo wfMessage('toc') . ':</a><div style="float:right;" class="collapse" id="collapsedTOC"><div class="card card-body">';
echo '<br><a href="#collapsedTOC" data-toggle="collapse">' . wfMessage('toc') . ':</a><div id="collapsedTOC" class="collapse">';
    for ($i = 0; $i < count($level); $i++){
	$sect_ = preg_replace('|\s|','_', $sect);
	echo '<br>' . str_repeat('&nbsp;&nbsp;&nbsp;&nbsp;', $level[$i]) . '<a href="#' .$sect_[$i]. '">' . $sect[$i] . '</a>';
    }
    echo '</div>';
}else{
    echo '<div style="float:right;"><div id="tweekiTOC"></div>';
}
/*
echo '<pre>'.print_r($this->get('sidebar')['TOOLBOX'], true).'</pre>';
echo '<pre>'.print_r($this->get('sidebar')['TOOLBOX']['shorturl'], true).'</pre>';
Array
(
    [id] => t-shorturl
    [href] => https://lahwiki.sphynkx.org.ua/r/2t6
    [title] => Скопируйте эту короткую ссылку
    [text] => Короткая ссылка
)
if (isset( $this->get('sidebar')['TOOLBOX']['shorturl'] )){
echo '<a href="' .$this->get('sidebar')['TOOLBOX']['shorturl']['href']. ' ' .$this->get('sidebar')['TOOLBOX']['shorturl']['title']. '">ShortUrl&#128279;</a>';
}
$shorturl = (isset( $this->get('sidebar')['TOOLBOX']['shorturl'] )) ? ' <a href="' .$this->get('sidebar')['TOOLBOX']['shorturl']['href']. '" title="' .$this->get('sidebar')['TOOLBOX']['shorturl']['title']. '">ShortUrl&#128279;</a>' : '';
echo $shorturl;
*/
...
<!-- mobile -->
<?php 
//$this->msg('Platform is ' . $_SERVER['HTTP_SEC_CH_UA_PLATFORM']);
?>
<?php if($_SERVER['HTTP_SEC_CH_UA_PLATFORM']==='"Android"'): ?>
<div style="border: 0px dotted #0f0; width: 300px;">
<?php endif; ?>
